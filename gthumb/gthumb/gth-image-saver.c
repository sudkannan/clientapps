...
#else
	GError  error;
	//file_saved_cb (file->buffer, file->buffer_size,&error,save_data);
	//fprintf(stdout, "filegetting saved %s", g_file_get_parse_name(file->file));
	_g_file_write (file->file, FALSE, G_FILE_CREATE_PRIVATE, file->buffer, file->buffer_size,
			save_data->data->cancellable, NULL);

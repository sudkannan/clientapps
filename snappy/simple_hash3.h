...
ht_node_create(ui64 key)
{
	...
	/*rqst.bytes = 1 * sizeof(struct ht_node);
	rqst.pid = PROC_ID;
	rqst.id++;
	node = (struct ht_node *)pnvmalloc(rqst.bytes, &rqst);
	assert(node);*/


    pthread_mutex_lock( &mutex1 );
    node = (struct ht_node *)npcalloc(1 , sizeof(struct ht_node));
    pthread_mutex_unlock( &mutex1 );
    if (node == 0)
    //if ((node = (struct ht_node *)npmalloc(1 * sizeof(struct ht_node))) == 0)
    {
		perror("calloc ht_node");
		assert(0);
		exit(1);
    }
...
ht_create(int size) 
{
	...
	rqst.bytes = 1 * sizeof(struct ht_ht);
	rqst.pid = PROC_ID;

	pthread_mutex_lock( &mutex1 );

	rqst.id++;
	//struct ht_ht *ht = (struct ht_ht *)npmalloc(1 * sizeof(struct ht_ht));
    struct ht_ht *ht = (struct ht_ht *)npcalloc(1, sizeof(struct ht_ht));
    ...
    ht->tbl			= (struct ht_node **)npcalloc(ht->size , sizeof(struct ht_node *));

	rqst.id++;
	rqst.bytes = ht->size * sizeof(struct ht_node *);
	total += ht->size * sizeof(struct ht_node *);
	pthread_mutex_unlock( &mutex1 );
